...
def build_optimizer(args, model):
    ...
    if flag == 'km_train':
        lr_en = args["lr_en"]
        lr_de = args["lr_de"]
        v_embed_params = list(map(id, model.vis_encoder.parameters()))
        c_params = list(map(id, model.classifier.parameters()))
        # v_en_params = list(map(id,model.vis_encoder.transformer.parameters()))
        de_params = filter(lambda x: id(x) not in v_embed_params and id(x) not in c_params , model.parameters())
        # de_params = filter(lambda x: id(x) not in v_embed_params, model.parameters())
        optimizer = getattr(torch.optim, optim)(
            [{'params': model.vis_encoder.parameters(), 'lr': lr_en, "lr_scale": 1.},
             {'params': model.classifier.parameters(), 'lr': lr_en, "lr_scale": 1.},
             {'params': de_params, 'lr': lr_de}],
            weight_decay=weight_decay,
            amsgrad=amsgrad,
        )
    elif flag == 'km_pretrain':
        lr_en = args["lr_en"]
        lr_de = args["lr_de"]
        v_embed_params = list(map(id, model.vis_encoder.vis_embed.parameters()))
        # v_en_params = list(map(id,model.vis_encoder.transformer.parameters()))
        # de_params = filter(lambda x: id(x) not in v_embed_params and id(x) not in rd_params , model.parameters())
        de_params = filter(lambda x: id(x) not in v_embed_params, model.parameters())
        optimizer = getattr(torch.optim, optim)(
            [{'params': model.vis_encoder.vis_embed.parameters(), 'lr': lr_en, "lr_scale": 1.},
             # {'params': model.vis_encoder.transformer.parameters(), 'lr': lr_en, "lr_scale": 1.},
             {'params': de_params, 'lr': lr_de}],
            weight_decay=weight_decay,
            amsgrad=amsgrad,
        )
    ...
    elif flag == 'r2gen_train':
        lr_ve = args["lr_ve"]
        lr_ed = args["lr_ed"]
        ve_params = list(map(id, model.visual_extractor.parameters()))
        ed_params = filter(lambda x: id(x) not in ve_params, model.parameters())
        optimizer = getattr(torch.optim, optim)(
            [{'params': model.visual_extractor.parameters(), 'lr': lr_ve},
             {'params': ed_params, 'lr': lr_ed}],
            weight_decay=weight_decay,
            amsgrad=amsgrad
        )
    else:
