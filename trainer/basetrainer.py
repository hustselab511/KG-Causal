...
class BaseTrainer(object):
    ...
    def _print_best_to_file(self):
        ...
        val_data = pd.DataFrame([self.best_recorder['val']])
        test_data = pd.DataFrame([self.best_recorder['test']])
        # 使用 pd.concat 替代 append
        record_table = pd.concat([record_table, val_data, test_data], ignore_index=True)
        # 保存表格
        record_table.to_csv(record_path, index=False)
